...
import { Tabs } from 'flowbite-react'; // Replace 'some-library' with the actual library name
import { MdOutlineArticle } from 'react-icons/md'; // Add this line to import MdOutlineArticle
import { FaLocationArrow } from 'react-icons/fa'; // Add this line to import FaLocationArrow
import UserPostCard from '../CustomComponent/UserPostCard'; 
import NetworkUserCard from '../CustomComponent/NetworkUserCard'; 
import { FaMailchimp, FaDotCircle } from 'react-icons/fa'; // Add this line to import FaMailchimp and FaDotCircle
    ...
    return (
        ...
        <div className="flex flex-col w-full mx-auto gap-4 md:px-0 px-2">
          ...
          <div className="flex flex-row justify-between items-center w-full rounded-lg overflow-hidden">
            <img src={user && user.profileBgPicture} className="w-full h-28 md:h-48 object-cover" alt="background picture" />
          </div>
          ...
          <div className="flex flex-row gap-4">
            ...
            <div className="w-1/4 md:w-1/6">
              <div className="relative w-20 h-20 md:w-32 md:h-32 self-center cursor-pointer shadow-md overflow-hidden rounded-full">
                <img src={user && user.profilePicture} alt="user" className="rounded-full w-full h-full object-cover" />
              </div>
            </div>
  
            <div className="w-full flex flex-col gap-1 justify-center items-start">
            <h1 className="text-2xl font-semibold">{user && user.channelName}</h1>
            <p className="flex flex-row justify-center items-center gap-2 md:text-sm text-xs"><span className="font-semibold">@{user?.username}</span> <FaDotCircle size={5}/> {user?.followers && user?.followers.length|| 0} Followers <FaDotCircle size={5}/> {userPostsLength} Posts</p>
            <p className="text-gray-500 text-sm font-medium">{user?.about}</p>
        ...
        <div className="flex flex-col bg-white rounded-md md:mx-0 mx-2 dark:bg-[rgb(16,23,42)]">
          <h1 className="text-md font-medium px-2 py-3 border-b">{user?.channelName}&apos;s Networks</h1>

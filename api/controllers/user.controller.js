...
export const updateUser = async (req, res, next) =>{
  ...
  if(req.body.username){
    if(req.body.username.length < 7 || req.body.username.length > 20 ){
        return next(errorHandler(400,'Username must be betweek 7 and 20 characters'));
    }
...
try{
res.clearCookie('acess_token').status(200).json('User has been signed out Successfuly.')
}catch(error){
 next(error)
}
}
